...
<div style="width: 1024px;height: 1280px;">
    ...
    <div class="mask_box">
        <div class="mask"></div>
        <div class="box" style="height:500">
            ...
            <label >金额合计</label>
            <div>
                <input type="text" class="layui-input" name="gwcAmt" readonly="readonly">
            </div>
            <div class="bot">
                <input type="submit"  onclick="cashOrder()" value="现金支付">
    ...
    <div class="layui-anim" id="dd" data-anim="layui-anim-fadein" style="height: 60px;width: 100%" onclick="gouwuche()"><#--<img src="/static/vmimgs/购物车2.png" style="width: 100px;height: 80px;float: right">-->
        <button id="gwcBtn" class="layui-btn layui-btn-fluid"  style="height: 70px;width: 1024px; background-color: #d9d9d9; font-size: 40px">购物车</button>
    </div>
...
<script>
    ...
    function goodsSlect(name,price,id,number){
        ...
        if(tableLength <= 0) {
            ...
            addtr += '<td class="number"><input type="hidden" value="' + id + '" class="goodsId">' + number + '</td>';//商品编码
            ...
            addtr += '<td class="heatFlag">' +
                    '<div class="Switch">\n' +
                    '   <input type="checkbox" class="isheat" name="Storage2" id="blance'+tableLength+'"/>\n' +
    ...
    $("#myTbody").on("click",".delete_btn",function(){
        ...
        if($("#myTbody tr").length < 1){
            ...
            $('#gwcBtn').css('background-color','#d9d9d9');
        }
    })
    ...
    function cashOrder() {
        ...
        $("#myTbody tr").each(function(){
            var orderDetail = {};
            //获取当前行商品ID
            orderDetail.goodsId = $(this).find(".goodsId").val();
            //获取当前行商品编号
            orderDetail.goodsNumber = $(this).children(".number").text();
            //获取当前行的单价
            orderDetail.goodsPrice = $(this).children(".danjia").text();
            //获取当前行的数量
            orderDetail.goodsQty = $(this).find(".num").val();
            //获取当前的加热标识
            if($(this).find(".isheat").val() =="on"){
                orderDetail.heatFlag = false;
            }else{
                orderDetail.heatFlag =true;
            }
            data.push(orderDetail);
        })
        if(data.length > 0){
            console.log(JSON.stringify(data));
            var orderMask = layer.msg('订单正在生成中，请稍候',{icon: 16,time:false,shade:0.8});
            $.ajax({
                type:"POST",
                url:"${base}/vem/order/orderByCash",
                dataType:"json",
                contentType:"application/json",
                data:JSON.stringify(data),
                success:function(orderResponse){
                    console.log(JSON.stringify(orderResponse));
                    layer.close(orderMask);
                    if(orderResponse.success){
                        var payMask = layer.msg('下单成功，请支付',{icon: 16,time:false,shade:0.8});
                        $.ajax({
                            type:"POST",
                            url:"${base}/vem/order/payByCash",
                            dataType:"json",
                            contentType:"application/json",
                            data:JSON.stringify(orderResponse.data),
                            success:function(payResponse){
                                console.log(JSON.stringify(payResponse));
                                layer.close(payMask);
                                if(payResponse.success){
                                    var payMask = layer.msg('支付成功，正在出货，请稍候',{icon: 16,time:false,shade:0.8});
                                    $.ajax({
                                        type:"POST",
                                        url:"${base}/vem/order/deliver",
                                        dataType:"json",
                                        contentType:"application/json",
                                        data:JSON.stringify(payResponse.data),
                                        success:function(deliverResponse){
                                            layer.close(payMask);
                                            if(deliverResponse.success){
                                                layer.msg("支付成功，请取出商品",{time: 3000},function(){
                                                    clearCloseGouwuche();
                                                });
                                            }else{
                                                layer.msg(deliverResponse.message);
                                            }

                                        }
                                    });
                                }else{
                                    layer.msg(payResponse.message);
                                }

                            }
                        });
                    }else{
                        layer.msg(orderResponse.message);
                    }
                }
            });
        }else{
            layer.msg("订单里没有数据",{time:1000});
        }
    }
